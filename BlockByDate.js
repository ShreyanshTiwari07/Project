...
module.exports = class {
  ...
  /**
 * Get the date information based on the provided date.
 * @param {string|Moment} date - The input date. It can be a string in a recognized date format or a Moment object.
 * @param {boolean} [after=true] - Flag indicating whether to return the date after or on the provided date. Default is true.
 * @param {boolean} [refresh=false] - Flag indicating whether to refresh the boundaries. Default is false.
 * @returns {Object} - The date information.
 * @property {string} date - The formatted date.
 * @property {number} blockNumber - The corresponding block number.
 */
async getDate(date, after = true, refresh = false) {
  if (!moment.isMoment(date)) {
    // Convert the input date to a Moment object if it's not already
    date = moment(date).utc();
  }

  if (typeof this.firstBlock === 'undefined' || typeof this.latestBlock === 'undefined' || typeof this.blockTime === 'undefined' || refresh) {
    // Fetch the boundaries if they are not defined or if refresh is requested
    await this.getBoundaries();
  }

  if (date.isBefore(moment.unix(this.firstBlock.timestamp))) {
    // If the date is before the first block, return the earliest date and block number
    return this.returnWrapper(date.format(), 1);
  }

  if (date.isSameOrAfter(moment.unix(this.latestBlock.timestamp))) {
    // If the date is on or after the latest block, return the latest date and block number
  ...
  async getEverySecond(duration, start, end, secondsToAdd = 1, after = true, refresh = false) {
  ...
  while (current.isSameOrBefore(end)) {
    ...
    current.add(secondsToAdd, 'seconds');
  }
  if (
  ...
  async getBlockWrapper(block) {
    ...
    this.savedBlocks[number] = {
      ...
      number: Number(number) // Convert BigInt to number
    };
    this.requests++;
